...
def gestisci_tracking_e_duplicati(ticker, fase, dist_attuale):
    """Ritorna (is_nuovo, is_migliorato, record_id)"""
    try:
        ...
        if not res.data:
            return True, False, None # Nuovo segnale assoluto
        
        record = res.data[0]
        dist_precedente = record.get('distanza_minima_raggiunta', 999.0)
        
        # Se il prezzo si è avvicinato sensibilmente (almeno lo 0.1% in più)
        if dist_attuale < (dist_precedente - 0.001):
            ...
            return False, True, record['id']
            
        return False, False, record['id'] # Già segnalato e non migliorato
...
for ticker in symbols:
    try:
        ...
        if distanza < SOGLIA_NOTIFICA:
            is_nuovo, is_migliorato, rec_id = gestisci_tracking_e_duplicati(ticker, fase_attuale, distanza)
            ...
            if is_nuovo or is_migliorato:
                ...
                msg = (f"{asset_info['cat']}\n{label}\n\n"
                       ...
                       f"📍 Distanza attuale: <b>{distanza:.2%}</b>\n"
                       f"{'✅ <i>Il prezzo si sta stringendo sul livello!</i>' if is_migliorato else ''}")
                
                send_telegram(msg, 'plot.png')
                
                if is_nuovo:
                    supabase.table("segnali_trading").insert({"ticker": ticker, "prezzo_ingresso": float(p_livello), "tp": float(tp), "sl": float(sl), "fase": fase_attuale, "distanza_minima_raggiunta": float(distanza)}).execute()
