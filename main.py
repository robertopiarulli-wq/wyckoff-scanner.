...
# Costanti Quantum/Wyckoff (sincronizzate con MQL5)
ALPHA = 0.00729735
...
for ticker in symbols:
    ...
    try:
        ...
        p_livello, sl, tp, is_acc = calcola_quantum_wyckoff(df)
        
        prezzo = df['Close'].iloc[-1]
        distanza = abs(prezzo - p_livello) / p_livello
        
        # Filtro: invia solo se la distanza è inferiore all'1.5%
        if distanza < 0.015:
